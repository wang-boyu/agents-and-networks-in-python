...
def redistribute_vertices(geom, distance):
    if isinstance(geom, LineString):
        ...
        raise TypeError(f"Wrong type: {type(geom)}. Must be LineString or MultiLineString.")
...
class UnitTransformer:
    ...
    def __init__(self, degree_crs=pyproj.CRS("EPSG:4326"), meter_crs=pyproj.CRS("EPSG:3857")):
        self._degree2meter = pyproj.Transformer.from_crs(degree_crs, meter_crs, always_xy=True)
        self._meter2degree = pyproj.Transformer.from_crs(meter_crs, degree_crs, always_xy=True)
