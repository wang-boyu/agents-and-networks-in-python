...
def plot_commuter_status_count(model_vars_df: pd.DataFrame) -> None:
    ...
    sns.relplot(x="time", y="count", data=commuter_status_df, kind="line", hue="status", aspect=1.5)
    plt.gca().xaxis.set_major_formatter(lambda x, pos: ":".join(str(datetime.timedelta(minutes=x)).split(":")[:2]))
...
def plot_num_friendships(model_vars_df: pd.DataFrame) -> None:
    ...
    friendship_df = friendship_df.melt(id_vars=["time"], value_vars=["home", "work"], var_name="friendship",
                                       value_name="count")
    sns.relplot(x="time", y="count", data=friendship_df, kind="line", hue="friendship", aspect=1.5)
    plt.gca().xaxis.set_major_formatter(lambda x, pos: ":".join(str(datetime.timedelta(minutes=x)).split(":")[:2]))
    plt.xticks(rotation=90)
